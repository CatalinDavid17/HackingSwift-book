...
//  Project28-keychain
//
//  Created by Catalin David on 27/09/16.
			...
			context.evaluatePolicy(.DeviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { [unowned self] (success: Bool, authenticationError: NSError?) -> Void in
				...
				dispatch_async(dispatch_get_main_queue()) {
					if success {
						...
						let ac = UIAlertController(title: "Authentication", message: "Your fingerprint could not be verified; please try again.", preferredStyle: .Alert)
						ac.addAction(UIAlertAction(title: "OK", style: .Default, handler: nil))
						self.presentViewController(ac, animated: true, completion: nil)
					}
				}

//
//  GameScene.swift
//  Project17 - ninja Swift\
//
//  Created by Catalin David on 30/08/16.
...
enum SequenceType: Int {
	case OneNoBomb, One, TwiWithOneBomb, Two, Three, Four, Chain, FastChain
}
...
class GameScene: SKScene {
    ...
    override func didMoveToView(view: SKView) {
		...
		sequence = [.OneNoBomb, .OneNoBomb, .TwiWithOneBomb, .TwiWithOneBomb, .Three, .One, .Chain]
...
extension GameScene {
	...
	private func createEnemy(forceBomb forceBomb: ForceBomb = .Default) {
...
extension GameScene {
	private func tossEnemies() {
		...
		chainDelay = chainDelay * 0.99
		physicsWorld.speed = physicsWorld.speed * 1.02
		...
		case .Chain:
			...
			RunAfterDelay(chainDelay / 5.0) { [unowned self] in
				self.createEnemy() }
			RunAfterDelay(chainDelay / 5.0 * 2) { [unowned self] in
				self.createEnemy() }
			RunAfterDelay(chainDelay / 5.0 * 3) { [unowned self] in
				self.createEnemy() }
			RunAfterDelay(chainDelay / 5.0 * 4) { [unowned self] in
				self.createEnemy() }
		case .FastChain:
			createEnemy()
			
			RunAfterDelay(chainDelay / 10.0) { [unowned self] in
				self.createEnemy()
			}
			RunAfterDelay(chainDelay / 10.0 * 2) { [unowned self] in
				self.createEnemy()
			}
			RunAfterDelay(chainDelay / 10.0 * 3) { [unowned self] in
				self.createEnemy()
			}
			RunAfterDelay(chainDelay / 10.0 * 4) { [unowned self] in
				self.createEnemy()
			}
		}
...
extension GameScene {
	...
	private func endGame(triggeredByBomb triggeredByBomb: Bool) {
		if gameEnded {
			return
		}

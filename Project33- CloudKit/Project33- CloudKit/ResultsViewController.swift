...
class ResultsViewController: UITableViewController {
    ...
    override func viewDidLoad() {
		...
		navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Download", style: .Plain, target: self, action: #selector(self.downloadTapped))
	...
	override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
		...
		ac.addAction(UIAlertAction(title: "Submit", style: .Default) { (action) -> Void in
			if suggestion.text?.characters.count > 0 {
				self.addSugestion(suggestion.text!)
			}
		})
	...
	func addSugestion(suggest: String) {
